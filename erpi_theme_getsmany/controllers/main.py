...
class erpiCompany(http.Controller):
    ...
    def index(self,  blog=None, tag=None, page=1, **opt):
        ...
        return http.request.render('erpi_theme_getsmany.catp_comp_home',{
            'posts': posts,
            # 'pager': pager,
            'blog_url': blog_url,
        })
    ...
    def about(self, **kw):
        return http.request.render('erpi_theme_getsmany.catp_comp_about')
    
    @http.route('/services', type='http',auth='public',website=True,page=True)
    def services(self, **kw):
        return http.request.render('erpi_theme_getsmany.catp_comp_services')
